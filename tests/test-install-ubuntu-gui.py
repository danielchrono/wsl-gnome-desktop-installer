...
check('rewrite nunca esvazia o rdp (leitura guardada)', "$c = Get-Content ''%RUNRDP%''" in src and "if ($c -match ''^full address:s:'')" in src)
...
check('rdp sem blob de senha (rdpsign deforma linha longa)', "'PasswordHex' not in src and 'password 51:b:' in src")
check('sidecar de credencial gravado na instalacao', "-Cred.txt" in src and 'CredTxtPath' in src)
check('helper le do sidecar com fallback sanitizado', 'PSCommandPath' in src and '[^0-9a-fA-F]' in src and 'Length % 2 -eq 1' in src)
check('rdp acompanha a janela (dynamic resolution)', "'dynamic resolution:i:1'" in src and "'smart sizing:i:1'" not in src)
...
check('publicador confiavel idempotente (store real)', ("'TrustedPublisher'" in src or '"TrustedPublisher"' in src) and 'TrustedPublishers' not in src and 'CN=Ubuntu-GUI RDP' in src)
check('publicador reusado so com chave privada (rdpsign 0x8009200B)', 'HasPrivateKey' in src)
check('reuso tambem confia (sem aviso mesmo assinado)', 'Publicador ja confiavel' in src)
check('ico validado pelo header (nao so Test-Path)', 'function Test-ValidIco' in src and 'Test-ValidIco -Path $IcoPath' in src)
check('ico valida entradas e dados (header sozinho refaz)', 'ToUInt32' in src and 'Test-ValidIco' in src)
check('atalho usa o ico so quando valido (senao mstsc)', '$icoSpec = if (Test-ValidIco' in src)
check('atalho PS com indice para o WScript (separa sozinho)', '"$IcoPath,0"' in src)
check('porta varre a faixa (sem queimar uma por rerun)', 'RdpScanExtra' in src and 'RdpWanted' in src and 'RdpFallbackPort' in src)
check('porta reaproveita o proprio RDP', 'reaproveitada' in src and 'Test-WslRdpListening' in src)
check('probe de porta nao invertido (livre usa)', "if (-not (Test-NetConnection -ComputerName '127.0.0.1' -Port $RDP_PORT -InformationLevel Quiet)) {" not in src)
check('tls confiavel no Root (sem aviso de rede)', 'Cert TLS confiavel' in src and "'Root'" in src)
check('assinatura detectada em UTF-8 e UTF-16 (rdpsign reescreve)', 'Unicode.GetString($rdpBytes)' in src)
check('assinatura re-verificada apos atalhos', 'Assinatura sumiu apos gravar atalhos' in src)
...
check('clique abre copia (mstsc nao invalida o original)', 'RUNRDP' in src and 'copy /y' in src and '"%MSTSC%" "%RUNRDP%"' in src)
check('reescrita dinamica so na copia', "Set-Content ''%RUNRDP%''" in src and "Set-Content ''%RDPPATH%''" not in src)
